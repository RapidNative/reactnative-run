...
When reactnative-esm bundles a package, it tracks the installed versions of all externalized dependencies. This information is returned as the `X-Externals` response header:

```
X-Externals: {"react":"19.1.0","react-dom":"19.1.0","memoize-one":"4.1.0"}
```

The browser-metro bundler reads this header and uses the pinned versions when fetching transitive dependencies, instead of defaulting to `@latest`. This prevents version mismatches where a transitive dep gets a different version than what the parent package was built against.
...
Bundled packages are cached to `reactnative-esm/cache/` with two files per package:
...
The server runs on port 5200 by default. The port is defined in `reactnative-esm/src/index.ts`.
...
npm run dev --prefix reactnative-esm

# Production
npm start --prefix reactnative-esm
```

## Integration with browser-metro

The bundler connects to reactnative-esm via the `server.packageServerUrl` config:
